...
public sealed class MainWindowViewModel : INotifyPropertyChanged
{
    ...
    private readonly ActiveMachineContext _activeMachineContext = new();
        ...
        string startupProjectFilePath)
    {
        ...
        _reelRuntimeAdapter = new MachineReelRuntimeAdapter(
            ...
            {
                FruitMachinePlatformType.MPU5 => ActiveRuntime?.Mpu5NativeRoms.Reels.FirstOrDefault(reel => reel.ReelIndex == reelId)?.Steps ?? System6ReelOptoSettings.DefaultSteps,
                FruitMachinePlatformType.Epoch => ActiveRuntime?.EpochNativeRoms.Reels.FirstOrDefault(reel => reel.ReelIndex == reelId)?.Steps ?? EpochReelSettings.DefaultSteps,
                FruitMachinePlatformType.Scorpion4 => ActiveRuntime?.Scorpion4Settings.Reels.FirstOrDefault(reel => reel.ReelIndex == reelId)?.Steps ?? 96,
    ...
    public IReadOnlyList<InputDefinitionModel> InputDefinitions => _activeMachineContext.Document?.InputDefinitions ?? [];
    ...
    public int DeleteInputDefinitions(IReadOnlyCollection<InputDefinitionModel> selectedInputs)
    {
        ...
        var currentInputs = (_activeMachineContext.Document?.InputDefinitions ?? []).ToList();
        var deletedCount = InputMapDeletionService.DeleteSelected(currentInputs, selectedInputs);
        if (deletedCount == 0)
        {
            return 0;
        }

        PersistActiveMachineInputDefinitions(currentInputs.ToArray());
    ...
    private MachineRuntimeDefinition? ActiveRuntime => _activeMachineContext.Document?.Runtime;
    ...
    public EditorProject? LoadedProject
    {
        ...
        private set
        {
            ...
            {
                Mpu3ProjectSettings = value is null ? null : new Mpu3ProjectSettingsViewModel(ActiveRuntime?.Mpu3Settings ?? new Mpu3ProjectSettings(), SaveMpu3ProjectSettings);
                M1ProjectSettings = value is null ? null : new M1ProjectSettingsViewModel(ActiveRuntime?.M1Settings ?? new M1ProjectSettings(), SaveM1ProjectSettings);
                Scorpion4ProjectSettings = value is null ? null : new Scorpion4ProjectSettingsViewModel(ActiveRuntime?.Scorpion4Settings ?? new Scorpion4ProjectSettings(), SaveScorpion4ProjectSettings);
    ...
    public DocumentTabViewModel? SelectedDocument
    {
        ...
        set
        {
            ...
            if (SetProperty(ref _selectedDocument, value))
            {
                ...
                UpdateActiveMachineFromSelectedDocument(value);
                NotifyInspectorChanged();
                NotifyDocumentCommands();
                RefreshHierarchy();
                NotifyHierarchyCommands();
    ...
    {
        _documentWorkspace.OpenMachineDocument();
    }
    ...
    private async void ImportMfmeFml()
    {
        ...
        try
        {
            ...
            {
                var merged = (_activeMachineContext.Document?.InputDefinitions ?? []).Concat(result.InputDefinitions).ToArray();
                PersistActiveMachineInputDefinitions(merged);
                OnPropertyChanged(nameof(InputDefinitions));
                RefreshInputMapDiagnostics();
                AddOutputEntry($"MFME FML import created {result.InputDefinitions.Count} input definitions on the active Machine.", OutputLogStatus.Info);
            }
    ...
    private PlayViewInputDispatcher? EnsurePlayViewInputDispatcher()
    {
        ...
        _playViewInputDispatcher ??= new PlayViewInputDispatcher(_playViewInputRouter!, _activeMachineContext.Document?.InputDefinitions ?? []);
    ...
    private EmulationLaunchRequest BuildEmulationLaunchRequest()
    {
        return SelectedFruitMachinePlatform switch
        {
            ...
            FruitMachinePlatformType.MPU5 => EmulationLaunchRequest.ForMpu5(
                ActiveRuntime?.Mpu5NativeRoms ?? new Mpu5NativeRomSettings(), BuildConfiguredLampIdsForLaunch(), BuildConfiguredSevenSegmentDisplayIdsForLaunch()),
            FruitMachinePlatformType.Epoch => EmulationLaunchRequest.ForEpoch(
                ActiveRuntime?.EpochNativeRoms ?? new EpochNativeRomSettings(), BuildConfiguredLampIdsForLaunch(), BuildConfiguredSevenSegmentDisplayIdsForLaunch()),
            FruitMachinePlatformType.MPU3 => EmulationLaunchRequest.ForMpu3(
                ActiveRuntime?.Mpu3Settings ?? new Mpu3ProjectSettings(), BuildConfiguredLampIdsForLaunch(), BuildConfiguredSevenSegmentDisplayIdsForLaunch()),
            FruitMachinePlatformType.MaygayM1 => EmulationLaunchRequest.ForM1(ActiveRuntime?.M1Settings ?? new M1ProjectSettings(), BuildConfiguredLampIdsForLaunch(), BuildConfiguredSevenSegmentDisplayIdsForLaunch()),
            FruitMachinePlatformType.Scorpion4 => EmulationLaunchRequest.ForScorpion4(ActiveRuntime?.Scorpion4Settings ?? new Scorpion4ProjectSettings(), BuildConfiguredLampIdsForLaunch(), BuildConfiguredSevenSegmentDisplayIdsForLaunch()),
    ...
    {
        var settings = ActiveRuntime?.Mpu5NativeRoms ?? new Mpu5NativeRomSettings();
        ...
        PersistActiveMachineRuntime(runtime => runtime with { Mpu5NativeRoms = settings });
    }
    ...
    private void SaveMpu5ProjectSettings(Mpu5NativeRomSettings settings)
    {
        PersistActiveMachineRuntime(runtime => runtime with { Mpu5NativeRoms = settings });
        SaveMpu5NativeRomSettings();
        Mpu5NativeRomStatus = "MPU5 Machine settings auto-saved.";
    }
    ...
    private void SaveSystem6NativeRomSettings()
    {
        ...
        PersistActiveMachineRuntime(runtime => runtime with { System6NativeRoms = settings });
    }
    ...
    {
        var settings = ActiveRuntime?.System6NativeRoms ?? new System6NativeRomSettings();
    ...
    private void LoadStartupProject(string startupProjectFilePath)
    {
        ...
        _activeMachineContext.TryAutoSelectFromProject(project);
    ...
    private void RefreshInputMapDiagnostics()
    {
        ...
        InputMapDiagnostics = _inputMapDiagnosticsService.Analyze(SelectedFruitMachinePlatform, _activeMachineContext.Document?.InputDefinitions ?? []);
        OnPropertyChanged(nameof(InputMapWarningCount));
        OnPropertyChanged(nameof(HasInputMapDiagnostics));
    ...
    {
        // Workspace-only project files no longer persist runtime platform settings or input definitions.
    }
    ...
    private static void WriteProjectSettings(Utf8JsonWriter writer, JsonElement existingProjectSettings, FruitMachinePlatformType platform, System6NativeRomSettings system6NativeRoms, Mpu5NativeRomSettings mpu5NativeRoms, EpochNativeRomSettings epochNativeRoms, Mpu3ProjectSettings mpu3Settings, M1ProjectSettings m1Settings, Scorpion4ProjectSettings scorpion4Settings)
    {
        ...
        foreach (var property in existingProjectSettings.EnumerateObject())
        {
            ...
            else if (property.NameEquals("EpochNativeRoms"))
            {
    ...
    private void SaveEpochProjectSettings(EpochNativeRomSettings settings) => PersistActiveMachineRuntime(runtime => runtime with { EpochNativeRoms = settings });

    private void SaveMpu3ProjectSettings(Mpu3ProjectSettings settings) => PersistActiveMachineRuntime(runtime => runtime with { Mpu3Settings = settings });

    private void SaveM1ProjectSettings(M1ProjectSettings settings) => PersistActiveMachineRuntime(runtime => runtime with { M1Settings = settings });

    private void SaveScorpion4ProjectSettings(Scorpion4ProjectSettings settings) => PersistActiveMachineRuntime(runtime => runtime with { Scorpion4Settings = settings });

    private static void WriteEpochNativeRomSettings(Utf8JsonWriter writer, EpochNativeRomSettings settings)
    {
        writer.WritePropertyName("EpochNativeRoms");
        JsonSerializer.Serialize(writer, settings);
    }

    private static EpochNativeRomSettings ResolveEpochNativeRomSettings(JsonElement root)
    {
    ...
    private void UpdateActiveMachineFromSelectedDocument(DocumentTabViewModel? document)
    {
        if (document?.Document.DocumentType == EditorDocumentType.Machine && !string.IsNullOrWhiteSpace(document.FilePath))
        {
            _activeMachineContext.SetActiveMachine(document.FilePath, document.GetMachineDocument());
            _activeMachineContext.ApplyRuntimeToDocumentState(document);
            SyncActiveMachineSettingsToViewModel();
            OnPropertyChanged(nameof(InputDefinitions));
            RefreshInputMapDiagnostics();
            return;
        ...
        if (document is null && _activeMachineContext.HasActiveMachine)
        {
            SyncActiveMachineSettingsToViewModel();
        }
    }

    private void SyncActiveMachineSettingsToViewModel()
    {
        var runtime = ActiveRuntime ?? MachineRuntimeDefinition.CreateDefault();
        _selectedFruitMachinePlatform = runtime.Platform;
        OnPropertyChanged(nameof(SelectedFruitMachinePlatform));
        ApplySystem6NativeRomSettingsToViewModel(runtime.System6NativeRoms);
        RefreshSystem6NativeRomStatus();
        ApplyMpu5NativeRomSettingsToViewModel(runtime.Mpu5NativeRoms);
        RefreshMpu5NativeRomStatus();
        EpochProjectSettings = new EpochProjectSettingsViewModel(runtime.EpochNativeRoms, SaveEpochProjectSettings);
        Mpu3ProjectSettings = LoadedProject is null ? null : new Mpu3ProjectSettingsViewModel(runtime.Mpu3Settings, SaveMpu3ProjectSettings);
        M1ProjectSettings = LoadedProject is null ? null : new M1ProjectSettingsViewModel(runtime.M1Settings, SaveM1ProjectSettings);
        Scorpion4ProjectSettings = LoadedProject is null ? null : new Scorpion4ProjectSettingsViewModel(runtime.Scorpion4Settings, SaveScorpion4ProjectSettings);
        OnPropertyChanged(nameof(InputDefinitions));
    }

    private void PersistActiveMachineRuntime(Func<MachineRuntimeDefinition, MachineRuntimeDefinition> mutate)
    {
        if (_activeMachineContext.Document is not { } document || string.IsNullOrWhiteSpace(_activeMachineContext.SelectedManifestPath))
        {
            return;
        }

        var nextRuntime = mutate(document.Runtime ?? MachineRuntimeDefinition.CreateDefault());
        var nextDocument = document with { Runtime = nextRuntime };
        _activeMachineContext.SetActiveMachine(_activeMachineContext.SelectedManifestPath!, nextDocument);
        var openMachine = OpenDocuments.FirstOrDefault(tab =>
        ...
        if (openMachine is not null)
        {
            openMachine.CommandService.Execute(MachineMutationCommands.CreateSetRuntimePlatformCommand(openMachine.DocumentId, openMachine, nextRuntime.Platform));
            if (nextRuntime.System6NativeRoms != document.Runtime.System6NativeRoms)
            {
                openMachine.CommandService.Execute(MachineMutationCommands.CreateSetSystem6NativeRomSettingsCommand(openMachine.DocumentId, openMachine, nextRuntime.System6NativeRoms));
            }
            if (nextRuntime.Mpu5NativeRoms != document.Runtime.Mpu5NativeRoms)
            {
                openMachine.CommandService.Execute(MachineMutationCommands.CreateSetMpu5NativeRomSettingsCommand(openMachine.DocumentId, openMachine, nextRuntime.Mpu5NativeRoms));
            }
            if (nextRuntime.EpochNativeRoms != document.Runtime.EpochNativeRoms)
            {
                openMachine.CommandService.Execute(MachineMutationCommands.CreateSetEpochNativeRomSettingsCommand(openMachine.DocumentId, openMachine, nextRuntime.EpochNativeRoms));
            }
            if (nextRuntime.Mpu3Settings != document.Runtime.Mpu3Settings)
            {
                openMachine.CommandService.Execute(MachineMutationCommands.CreateSetMpu3SettingsCommand(openMachine.DocumentId, openMachine, nextRuntime.Mpu3Settings));
            }
            if (nextRuntime.M1Settings != document.Runtime.M1Settings)
            {
                openMachine.CommandService.Execute(MachineMutationCommands.CreateSetM1SettingsCommand(openMachine.DocumentId, openMachine, nextRuntime.M1Settings));
            }
            if (nextRuntime.Scorpion4Settings != document.Runtime.Scorpion4Settings)
            {
                openMachine.CommandService.Execute(MachineMutationCommands.CreateSetScorpion4SettingsCommand(openMachine.DocumentId, openMachine, nextRuntime.Scorpion4Settings));
            }
            _activeMachineContext.SetActiveMachine(_activeMachineContext.SelectedManifestPath!, openMachine.GetMachineDocument());
        }
        else if (File.Exists(_activeMachineContext.SelectedManifestPath))
        {
            File.WriteAllText(_activeMachineContext.SelectedManifestPath, MachineDocumentStorage.Serialize(nextDocument));
        }
    }

    private void PersistActiveMachineInputDefinitions(InputDefinitionModel[] inputDefinitions)
    {
        if (_activeMachineContext.Document is not { } document || string.IsNullOrWhiteSpace(_activeMachineContext.SelectedManifestPath))
        {
            return;
        ...
        var nextDocument = document with { InputDefinitions = inputDefinitions };
        _activeMachineContext.SetActiveMachine(_activeMachineContext.SelectedManifestPath!, nextDocument);
        var openMachine = OpenDocuments.FirstOrDefault(tab =>
            tab.Document.DocumentType == EditorDocumentType.Machine
            && string.Equals(Path.GetFullPath(tab.FilePath), Path.GetFullPath(_activeMachineContext.SelectedManifestPath!), StringComparison.OrdinalIgnoreCase));
        if (openMachine is not null)
        {
            openMachine.CommandService.Execute(MachineMutationCommands.CreateSetInputDefinitionsCommand(openMachine.DocumentId, openMachine, inputDefinitions));
            _activeMachineContext.SetActiveMachine(_activeMachineContext.SelectedManifestPath!, openMachine.GetMachineDocument());
        }
        else if (File.Exists(_activeMachineContext.SelectedManifestPath))
        {
