...
internal static class FaceMutationCommands
{
    ...
    private sealed class SetArtworkSourceMutationCommand : Commands.IDocumentCommand, Commands.IExecutionTrackedCommand
    {
        ...
        public void Execute(){var model=_document.GetFaceDocument();if(model.Artwork is null)return;_previous??=model.Artwork.Source;
            ...
            _document.SetFaceDocument(WithArtworkSource(model,_next),CreateChange(_document,null,PanelChangeProperties.Metadata|PanelChangeProperties.Structure,true));if(_document.FaceArtworkRegistrationEditing)_document.FaceArtworkRegistrationPreview=_next.RegistrationQuad;_document.MarkDirty();WasExecuted=true;}
        public void Undo(){if(_previous is null)return;_document.SetFaceDocument(WithArtworkSource(_document.GetFaceDocument(),_previous),CreateChange(_document,null,PanelChangeProperties.Metadata|PanelChangeProperties.Structure,true));if(_document.FaceArtworkRegistrationEditing)_document.FaceArtworkRegistrationPreview=_previous.RegistrationQuad;_document.MarkDirty();}
