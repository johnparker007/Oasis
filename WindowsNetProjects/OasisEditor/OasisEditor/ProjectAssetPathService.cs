...
public sealed class ProjectAssetPathService
{
    ...
    public static string? NormalizeAssetPackageDirectoryPath(string? path)
    {
        ...
        if (normalized.EndsWith("/" + Cabinet3DManifestFileName, StringComparison.OrdinalIgnoreCase))
        {
            normalized = normalized[..^(Cabinet3DManifestFileName.Length + 1)];
        }

        return normalized.TrimEnd('/');
    }
    private static string GetAssetTypeFolderName(EditorAssetType assetType) => assetType switch { EditorAssetType.Panel2D => "Panel2D", EditorAssetType.Face => "Faces", EditorAssetType.Cabinet3D => "Cabinet3D", EditorAssetType.Machine => "Machines", _ => throw new ArgumentOutOfRangeException(nameof(assetType), assetType, null) };
    private static string GetManifestFileName(EditorAssetType assetType) => assetType switch { EditorAssetType.Panel2D => Panel2DManifestFileName, EditorAssetType.Face => FaceManifestFileName, EditorAssetType.Cabinet3D => Cabinet3DManifestFileName, EditorAssetType.Machine => MachineManifestFileName, _ => throw new ArgumentOutOfRangeException(nameof(assetType), assetType, null) };
}
