...
public sealed class InspectorViewModel : INotifyPropertyChanged
{
    ...
    private void RebuildCabinetDocumentPropertyRows(DocumentTabViewModel selectedDocument)
    {
        ...
        _propertyRows.Add(new InspectorInfoPropertyViewModel("Machine", "Composition", "Surface and reel assignments are edited on the Machine asset."));
    ...
    private static IReadOnlyList<string> DiscoverFaceAssetPaths(EditorProject? project)
    {
        ...
        if (selectedElement is FaceArtworkElement artwork)
        {
            ...
            for (var index = 0; index < authoredArtwork.ProcessingPipeline.Operations.Count; index++)
            {
                ...
                foreach(var colourGroup in calibration.SameColorGroups)
                {
                    ...
                    _propertyRows.Add(new InspectorActionPropertyViewModel($"Add {colourGroup.Name} Sample",group,new RelayCommand(()=>selectedDocument.BeginCalibrationPlacement(new(calibration.Id,CalibrationPlacementTargetKind.SameColorGroup,colourGroup.Id,CalibrationSamplingMode.Pixel,.01)))));
                ...
                void AddReference(string name,CalibrationReferenceModel reference,string? derived,CalibrationPlacementTargetKind kind)
                {
                    ...
                    _propertyRows.Add(new InspectorActionPropertyViewModel($"Add {name} Sample",group,new RelayCommand(()=>selectedDocument.BeginCalibrationPlacement(new(calibration.Id,kind,string.Empty,CalibrationSamplingMode.Pixel,.01)))));
                }
