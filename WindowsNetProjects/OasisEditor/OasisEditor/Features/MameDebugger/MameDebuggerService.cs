...
public sealed class MameDebuggerService
{
    ...
    private async Task<T> SendRequestAsync<T>(string op, object? parameters = null, CancellationToken cancellationToken = default)
    {
        ...
        var responseTask = _responseRouter.Register(requestId, timeout.Token);

        _diagnosticLogger?.Invoke($"[MAME-DEBUGGER-OUT] {commandLine}");
        await _transport.SendAsync(commandLine, timeout.Token).ConfigureAwait(false);
        ...
        {
            throw new TimeoutException($"Timed out waiting for MAME debugger response to operation '{op}'.");
        }
