...
public sealed class CabinetReflectionEditorViewModel : INotifyPropertyChanged, IDisposable
{
    ...
    public CabinetReflectionEditorViewModel(DocumentTabViewModel document, Func<EditorProject?>? projectAccessor = null)
    {
        ...
        AddSourceCommand = new RelayCommand(AddSource, () => Selected is not null && Selected.Sources.Length < CabinetReflectionContract.MaximumSources && SurfaceTargetChoices.Count > 0);
    ...
    public ObservableCollection<CabinetReflectionSurfaceTargetChoice> SurfaceTargetChoices { get; } = new();
    ...
    public void Refresh()
    {
        ...
        List<CabinetReflectionSurfaceTargetChoice> discovered;
        try { discovered = CabinetReflectionFaceCatalog.DiscoverSurfaceTargets(_document.GetCabinetDocument()).ToList(); }
        ...
        foreach (var missing in storedIds.Where(id => discovered.All(choice => choice.TargetId != id)).Distinct(StringComparer.Ordinal)) discovered.Add(new(missing, $"Missing target ({missing})", $"Missing target ({missing})", true));
        SurfaceTargetChoices.Clear(); foreach (var choice in discovered) SurfaceTargetChoices.Add(choice);
    ...
    private void StartWatcher()
    {
        ...
        var modelPath = _document.GetCabinetDocument().Model.Path;
        ...
        try
        {
            ...
            _watcher = new FileSystemWatcher(directory) { IncludeSubdirectories = false, NotifyFilter = NotifyFilters.FileName | NotifyFilters.LastWrite };
            FileSystemEventHandler changed = (_, _) => ScheduleRefresh();
            _watcher.Changed += changed; _watcher.Created += changed; _watcher.Renamed += (_, _) => ScheduleRefresh(); _watcher.EnableRaisingEvents = true;
        }
    ...
    private void Add() { var target = Targets.FirstOrDefault(); if (target is null) return; var sources = SurfaceTargetChoices.FirstOrDefault(choice => !choice.IsMissing) is { } choice ? new[] { NewSource(choice.TargetId) } : []; var item = new CabinetReflectionDefinition("reflection-" + Guid.NewGuid().ToString("N"), target.TargetPath, 0, sources, CabinetReflectionSettings.RoughPlastic); ExecuteAdd(item); if (Sources.Count > 0) Derive(); }
    ...
    private static CabinetReflectionSource NewSource(string targetId) => new(targetId, new(new(0, 0, 0), new(1, 0, 0), new(0, 1, 0), 1, 1));
    ...
    private void AddSource() { if (Selected is null) return; var choice = SurfaceTargetChoices.FirstOrDefault(candidate => !candidate.IsMissing && Selected.Sources.All(source => source.SourceSurfaceTargetId != candidate.TargetId)); if (choice is null) return; Update(item => item with { Sources = item.Sources.Append(NewSource(choice.TargetId)).ToArray() }); SelectedSource = Sources.LastOrDefault(); Derive(); }
    ...
    private void Derive() { if (SelectedSource is null) return; var target = FaceTargets.FirstOrDefault(item => item.Id == SelectedSource.SourceSurfaceTargetId); if (CabinetReflectionPlaneDeriver.TryDerive(target, out var plane, out _)) UpdateSource(SelectedSource.Index, source => source with { Plane = plane, PlaneSource = CabinetReflectionPlaneSource.Automatic }); }
...
public sealed class CabinetReflectionSourceViewModel : INotifyPropertyChanged
{
    ...
    public CabinetReflectionSurfaceTargetChoice? Choice { get => _owner.SurfaceTargetChoices.FirstOrDefault(item => item.TargetId == Model.SourceSurfaceTargetId); set { if (value is not null) _owner.UpdateSource(Index, source => source with { SourceSurfaceTargetId = value.TargetId }); } }
