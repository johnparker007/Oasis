...
{
 [Fact] public void SchemaAndDefaults(){Assert.Equal(7,EditorProject.CurrentSchemaVersion);Assert.True(Enum.IsDefined(FruitMachinePlatformType.M1));var a=new M1ProjectSettings();var b=new M1ProjectSettings();Assert.Equal(4,a.ProgramRoms.Count);Assert.Equal(4,a.SoundRoms.Count);Assert.Equal(6,a.Reels.Count);Assert.Equal(16,a.Dips.Count);Assert.Equal(2,a.Hoppers.Count);a.Dips[0]=true;Assert.False(b.Dips[0]);Assert.All(a.Reels,r=>{Assert.Equal(96,r.Steps);Assert.Equal(4,r.OptoEnd);});}
 ...
 [Fact] public void VisualPolicyIsExplicit(){Assert.True(AlphaCellOrder.IsAmberBackedPlatform(FruitMachinePlatformType.M1));Assert.False(PlatformReelDirectionResolver.RequiresReversal(FruitMachinePlatformType.M1));Assert.Equal(0,InternalReelOffsetResolver.ResolveNormalizedOffset(FruitMachinePlatformType.M1,12));}
