...
Open Face state is a content source, not an existence source. The builder first requires the explicitly referenced Face manifest to exist at its authored project path; only then may a matching open tab supply current graph-relevant provenance or Reel-mount state instead of the saved file. Face document titles remain the package identity established by the existing document model. If the manifest disappears, the Face stays connected to its Cabinet target as a Missing node and stale open content contributes neither provenance nor Reel edges. Restoring the manifest allows the next existing catalog refresh to use the open model again. The stale Face tab is neither closed nor modified, and Machine composition/dirty/undo state is untouched. Panel2D provenance already independently requires its manifest to exist on disk.
