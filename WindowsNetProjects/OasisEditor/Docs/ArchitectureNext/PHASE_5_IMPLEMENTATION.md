...
The intentionally single creation workflow is **Copy to Oasis Library** on a project Cabinet or Reel package in the Assets pane. Both packages are validated against their current authored formats before copying: Cabinets additionally require a contained GLB and contained reflection resources, while Reels must pass the current Reel reader's ID, display-name, schema, and physical-dimension validation. The workflow then copies the entire already-self-contained package to `Cabinets/<name>` or `Reels/<name>`, including Cabinet GLB, reflection masks, and supporting files. Existing destinations are never silently overwritten; the user is prompted for a different package name. Copying does not move the source, rewrite Machine references, or dirty documents. The new Library root in the same pane permits browsing, opening, Show in Explorer, and in-place Save back to the existing Library manifest. Opening never imports or silently copies the asset into a project.
