...
Machine Reel rows are derived from saved assigned Face assets. A Face-assignment mutation immediately recomputes only the Reel-row topology, preserving unaffected rows and authored mappings; undo and redo perform the same targeted recomputation. Assigned Face manifest stamps participate in the Machine dependency signature, so saving an assigned Face and receiving the normal asset-catalog refresh updates its required logical reels. Unsaved changes in an open Face are intentionally ignored until save in Phase 3, while unrelated Face files are not stamped or traversed.
