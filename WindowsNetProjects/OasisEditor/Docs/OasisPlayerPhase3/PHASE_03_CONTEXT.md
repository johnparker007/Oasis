...
`FaceRotation` and `FaceFlipHorizontal` are now exported and applied by Oasis Player as UV-only orientation settings. They do not alter `_Cull`, `_OasisNormalSign`, cabinet geometry, or which physical side renders. Manual validation found the Player's initial rotation support was active but offset by 90 degrees; the root cause is that Editor rotation reorders detected target corners with fixed texture coordinates, while Player transforms imported GLB UVs whose baseline ordering is already one quarter turn ahead. The centralized Player mapping compensates for that baseline difference without changing exported values.
