...
Oasis Editor Face orientation controls are target-specific cabinet settings. `CabinetTargetOverride` stores `FrontSide` (`normal`/`inverted`), `FaceRotation`, and `FaceFlipHorizontal`, and the Editor preview uses those values to reorder quad corners and reverse winding. Runtime build export now writes the normalized `FrontSide` value as `frontSide` on each `machine.runtime.json` Face reference, alongside `faceId`, `assetName`, `cabinetFaceTargetId`, and `manifest`, so the setting remains a Face-to-cabinet-target binding rather than an intrinsic `face.runtime.json` artwork asset property.

Oasis Player loads that field into `MachineRuntimeFaceReference.frontSide` and interprets it through the `RuntimeFaceFrontSide` enum/helper instead of renderer-side raw string comparisons. `normal` uses the `Oasis/Face` shader with explicit back-face culling and a normal sign of `1`; `inverted` uses the `Oasis/FaceInverted` shader with explicit front-face culling and a normal sign of `-1`. The runtime material still writes and verifies the diagnostic `_Cull` value, but raster side selection no longer depends on `Cull [_Cull]` after manual testing showed the material-controlled culling path did not visibly switch sides. The normal sign is applied once before ambient/main/additional-light evaluation so the inverted side is treated as the visible lit front. No fragment shader front/back branching, double-sided culling, `abs(dot())`, lamp lookup, lamp texture, transparency, or artwork export changes are part of this orientation path. Missing required orientation shader properties now fail material creation with a clear warning instead of silently falling back to defaults.

`FaceRotation` and `FaceFlipHorizontal` were confirmed to be Editor preview settings only in this path. They are still not exported or applied by Oasis Player; carrying them through a future target-binding schema extension remains deferred work.
