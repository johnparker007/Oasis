...
`FaceRotation` and `FaceFlipHorizontal` are now exported and applied by Oasis Player as UV-only orientation settings. They do not alter `_Cull`, `_OasisNormalSign`, cabinet geometry, or which physical side renders. Manual validation found the Player's initial rotation support was active but did not match Editor semantics because the Unity UV baseline and positive rotation direction differ from the Editor preview. The root cause is that Editor rotation reorders detected target corners with fixed texture coordinates, while Player transforms imported GLB UVs. The centralized Player mapping reflects the rotation direction and compensates for the baseline difference without changing exported values.
