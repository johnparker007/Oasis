...
Oasis Player loads that field into `MachineRuntimeFaceReference.frontSide` and interprets it through the `RuntimeFaceFrontSide` enum/helper instead of renderer-side raw string comparisons. `normal` configures each runtime-owned Face material with back-face culling and a normal sign of `1`; `inverted` configures front-face culling and a normal sign of `-1`. The `Oasis/Face` shader remains single-sided via material-controlled `Cull [_Cull]`, and the normal sign is applied once before ambient/main/additional-light evaluation so the inverted side is treated as the visible lit front. No fragment shader front/back branching, double-sided culling, `abs(dot())`, lamp lookup, lamp texture, transparency, or artwork export changes are part of this orientation path.
