...
Fabric advancement is now driven by an Oasis Editor-owned runtime update loop running on the thread pool, not by WPF rendering or a UI `DispatcherTimer`. Each eligible editor update calls `IEditorUpdateDrivenEmulationBackend.UpdateAsync`, which measures monotonic elapsed time inside `FabricEmulationBackend`, clamps a single advance to the native display-frame period, processes inputs, publishes a snapshot, and drains available PCM into the existing NAudio sink. The previous independent fixed 1 kHz Fabric pump is intentionally not started in this elapsed-update mode, so there is only one session advancer.
