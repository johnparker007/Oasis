Shader "Oasis/CabinetAnalyticReflection"
{
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            TEXTURE2D(_OasisArtworkTex0); SAMPLER(sampler_OasisArtworkTex0); TEXTURE2D(_OasisMaskTex0); SAMPLER(sampler_OasisMaskTex0); TEXTURE2D(_OasisLampIds0Tex0); SAMPLER(sampler_OasisLampIds0Tex0); TEXTURE2D(_OasisLampWeights0Tex0); SAMPLER(sampler_OasisLampWeights0Tex0);
            TEXTURE2D(_OasisArtworkTex1); SAMPLER(sampler_OasisArtworkTex1); TEXTURE2D(_OasisMaskTex1); SAMPLER(sampler_OasisMaskTex1); TEXTURE2D(_OasisLampIds0Tex1); SAMPLER(sampler_OasisLampIds0Tex1); TEXTURE2D(_OasisLampWeights0Tex1); SAMPLER(sampler_OasisLampWeights0Tex1);
            TEXTURE2D(_OasisArtworkTex2); SAMPLER(sampler_OasisArtworkTex2); TEXTURE2D(_OasisMaskTex2); SAMPLER(sampler_OasisMaskTex2); TEXTURE2D(_OasisLampIds0Tex2); SAMPLER(sampler_OasisLampIds0Tex2); TEXTURE2D(_OasisLampWeights0Tex2); SAMPLER(sampler_OasisLampWeights0Tex2);
            TEXTURE2D(_OasisArtworkTex3); SAMPLER(sampler_OasisArtworkTex3); TEXTURE2D(_OasisMaskTex3); SAMPLER(sampler_OasisMaskTex3); TEXTURE2D(_OasisLampIds0Tex3); SAMPLER(sampler_OasisLampIds0Tex3); TEXTURE2D(_OasisLampWeights0Tex3); SAMPLER(sampler_OasisLampWeights0Tex3);
            ...
            float2 RotateUv(float2 uv,float turns,float flip) { turns=floor(turns+0.5); if(turns==1)uv=float2(1-uv.y,uv.x);else if(turns==2)uv=1-uv;else if(turns==3)uv=float2(uv.y,1-uv.x);if(flip>=0.5)uv.x=1-uv.x;return uv; }
            float3 ReconstructFace0(float2 uv) { uv=RotateUv(uv,_OasisFaceRotationQuarterTurns0,_OasisFaceFlipHorizontal0); half4 art=SAMPLE_TEXTURE2D(_OasisArtworkTex0,sampler_OasisArtworkTex0,uv); half4 mask=SAMPLE_TEXTURE2D(_OasisMaskTex0,sampler_OasisMaskTex0,uv); half4 ids=SAMPLE_TEXTURE2D(_OasisLampIds0Tex0,sampler_OasisLampIds0Tex0,uv); half4 weights=SAMPLE_TEXTURE2D(_OasisLampWeights0Tex0,sampler_OasisLampWeights0Tex0,uv); float amount=saturate(max(mask.r,max(mask.g,mask.b))*mask.a*_OasisMaskStrength0)*saturate(Lamp(ids.r)*weights.r+Lamp(ids.g)*weights.g+Lamp(ids.b)*weights.b); float3 exposed=art.rgb*exp2(max(_OasisLampExposureStops0,0)*amount); return art.rgb*_OasisReflectionUnlitArtworkStrength+max(exposed-art.rgb,0)*_OasisReflectionLitLampStrength; }
            float3 ReconstructFace1(float2 uv) { uv=RotateUv(uv,_OasisFaceRotationQuarterTurns1,_OasisFaceFlipHorizontal1); half4 art=SAMPLE_TEXTURE2D(_OasisArtworkTex1,sampler_OasisArtworkTex1,uv); half4 mask=SAMPLE_TEXTURE2D(_OasisMaskTex1,sampler_OasisMaskTex1,uv); half4 ids=SAMPLE_TEXTURE2D(_OasisLampIds0Tex1,sampler_OasisLampIds0Tex1,uv); half4 weights=SAMPLE_TEXTURE2D(_OasisLampWeights0Tex1,sampler_OasisLampWeights0Tex1,uv); float amount=saturate(max(mask.r,max(mask.g,mask.b))*mask.a*_OasisMaskStrength1)*saturate(Lamp(ids.r)*weights.r+Lamp(ids.g)*weights.g+Lamp(ids.b)*weights.b); float3 exposed=art.rgb*exp2(max(_OasisLampExposureStops1,0)*amount); return art.rgb*_OasisReflectionUnlitArtworkStrength+max(exposed-art.rgb,0)*_OasisReflectionLitLampStrength; }
            float3 ReconstructFace2(float2 uv) { uv=RotateUv(uv,_OasisFaceRotationQuarterTurns2,_OasisFaceFlipHorizontal2); half4 art=SAMPLE_TEXTURE2D(_OasisArtworkTex2,sampler_OasisArtworkTex2,uv); half4 mask=SAMPLE_TEXTURE2D(_OasisMaskTex2,sampler_OasisMaskTex2,uv); half4 ids=SAMPLE_TEXTURE2D(_OasisLampIds0Tex2,sampler_OasisLampIds0Tex2,uv); half4 weights=SAMPLE_TEXTURE2D(_OasisLampWeights0Tex2,sampler_OasisLampWeights0Tex2,uv); float amount=saturate(max(mask.r,max(mask.g,mask.b))*mask.a*_OasisMaskStrength2)*saturate(Lamp(ids.r)*weights.r+Lamp(ids.g)*weights.g+Lamp(ids.b)*weights.b); float3 exposed=art.rgb*exp2(max(_OasisLampExposureStops2,0)*amount); return art.rgb*_OasisReflectionUnlitArtworkStrength+max(exposed-art.rgb,0)*_OasisReflectionLitLampStrength; }
            float3 ReconstructFace3(float2 uv) { uv=RotateUv(uv,_OasisFaceRotationQuarterTurns3,_OasisFaceFlipHorizontal3); half4 art=SAMPLE_TEXTURE2D(_OasisArtworkTex3,sampler_OasisArtworkTex3,uv); half4 mask=SAMPLE_TEXTURE2D(_OasisMaskTex3,sampler_OasisMaskTex3,uv); half4 ids=SAMPLE_TEXTURE2D(_OasisLampIds0Tex3,sampler_OasisLampIds0Tex3,uv); half4 weights=SAMPLE_TEXTURE2D(_OasisLampWeights0Tex3,sampler_OasisLampWeights0Tex3,uv); float amount=saturate(max(mask.r,max(mask.g,mask.b))*mask.a*_OasisMaskStrength3)*saturate(Lamp(ids.r)*weights.r+Lamp(ids.g)*weights.g+Lamp(ids.b)*weights.b); float3 exposed=art.rgb*exp2(max(_OasisLampExposureStops3,0)*amount); return art.rgb*_OasisReflectionUnlitArtworkStrength+max(exposed-art.rgb,0)*_OasisReflectionLitLampStrength; }
            float3 ReconstructFace(int source,float2 uv) { if(source==0)return ReconstructFace0(uv);if(source==1)return ReconstructFace1(uv);if(source==2)return ReconstructFace2(uv);return ReconstructFace3(uv); }
            bool HitPlane(float3 origin,float3 ray,float4 planeOrigin,float4 right,float4 up,float4 normal,float4 size,out float t,out float2 uv) { t=0;uv=0;if(size.x<=0||size.y<=0)return false;float d=dot(ray,normal.xyz);if(abs(d)<1e-5)return false;t=dot(planeOrigin.xyz-origin,normal.xyz)/d;if(t<=1e-5)return false;float3 relative=origin+ray*t-planeOrigin.xyz;uv=float2(dot(relative,right.xyz)/size.x,dot(relative,up.xyz)/size.y);return !any(uv<0)&&!any(uv>1); }
