...
namespace OasisPlayer.RuntimeBuild
{
    ...
    public sealed class RuntimeSegmentDisplayRenderer
    {
        ...
        private void RenderEntry(RuntimeMachine machine, RuntimeFace face, RuntimeFaceSurfaceGeometry surface, FaceRuntimeSegmentDisplayManifestEntry e)
        {
            ...
            for (var visualIndex = 0; visualIndex < count; visualIndex++) { var go = new GameObject("Digit_" + visualIndex); go.transform.SetParent(root.transform, false); var mf = go.AddComponent<MeshFilter>(); var mr = go.AddComponent<MeshRenderer>(); mf.sharedMesh = mesh; mr.sharedMaterial = material; var cx = e.x + cellW * (visualIndex + .5f); var cy = e.y + e.height * .5f; go.transform.position = surface.FacePointToWorld(cx, cy, face.Manifest.width, face.Manifest.height) + surface.VisibleNormal * RuntimeFacePlacement.DefaultSurfaceClearanceMetres; go.transform.rotation = Quaternion.LookRotation(-surface.VisibleNormal, surface.VerticalTangent); go.transform.localScale = new Vector3(surface.PhysicalWidth * cellW / face.Manifest.width, surface.PhysicalHeight * e.height / face.Manifest.height, 1f); var b = new RuntimeSegmentDigitBinding(mr, e.machineReference, visualIndex, Parse(e.onColorHex, Color.red), Parse(e.offColorHex, new Color(.04f,0,0,1))); _digits.Add(b); b.Apply(0, 1f); }
        }
