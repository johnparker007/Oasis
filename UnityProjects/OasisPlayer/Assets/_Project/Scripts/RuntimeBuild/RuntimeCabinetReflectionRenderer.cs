    ...
    public sealed class RuntimeCabinetReflectionRenderer
    {
        public void Render(RuntimeMachine machine)
        {
            ...
            foreach (var definition in definitions)
            {
                ...
                for (var sourceIndex = 0; sourceIndex < sourceDefinitions.Length; sourceIndex++) { var source = sourceDefinitions[sourceIndex]; if (source == null || !TryWorldPlane(machine.Cabinet.transform, source.plane, out var plane)) { Warn(machine, definition, $"source {sourceIndex} cabinet-local Face plane is invalid after world transformation"); sourceFailure = true; break; } var reference = machine.Faces.FirstOrDefault(face => face.Reference != null && string.Equals(face.Reference.cabinetFaceTargetId, source.sourceSurfaceTargetId, StringComparison.Ordinal))?.Reference; var frontSide = RuntimeFaceFrontSideExtensions.Parse(reference != null ? reference.frontSide : null); sources[sourceIndex] = new RuntimeCabinetReflectionSource(source.sourceSurfaceTargetId, plane, frontSide); }
